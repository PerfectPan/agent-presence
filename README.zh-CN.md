...
`@rivus/agent-presence` 的核心是 presence，而不是飞书专用逻辑。默认输出是 `magic.solutionsuite.cn` 上的 **magic-builder** FaaS 预览：presence 值写入 `l.garyyang` slot，每次飞书拉取预览时由 FaaS 在服务端读取该值。之所以默认走 magic-builder，是因为飞书可能不渲染 `l.garyyang.work` 直连预览页。
...
> `l.garyyang.work` 直连预览（`--provider feishu-signature`，无需 Magic-Builder token）是遗留备选，飞书可能已经不再渲染它。仍想用的话见 [Providers 文档](https://agent-presence.vercel.app/zh/guide/providers/)。
...
- [Providers](https://agent-presence.vercel.app/zh/guide/providers/) —— magic-builder（默认）与 l.garyyang 直连预览

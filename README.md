...
Surface it in the signature with render variables (`{usage_1d}`, `{usage_7d}`) or `usage.showInSignature: true`. The first cache-building boundary and the first session boundary after midnight refresh all built-in contributions so unused sources count as zero; later same-day boundaries refresh only their owning source. Failed scans leave the previous cache retryable, and overlapping refreshes cannot replace newer snapshots with older results. Deferred provider flushes publish that cache without rescanning, while `agent-presence update` performs an explicit full refresh. The standalone `usage` command always performs a live full scan, while the signature intentionally reflects the latest source-boundary snapshots. Supported models are priced from a bundled LiteLLM snapshot, including model-specific cache TTL rates, Codex fast-tier multipliers, and whole-request long-context rates above 272K input tokens; copied Codex branch events are de-duplicated across session files (private models can still use pricing overrides). Sources and the stale-badge `—` guard: [Token usage](https://agent-presence.vercel.app/guide/token-usage/).

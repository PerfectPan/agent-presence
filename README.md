...
Surface it in the signature with render variables (`{usage_1d}`, `{usage_7d}`) or `usage.showInSignature: true`. The first session boundary after midnight refreshes all built-in contributions so unused sources count as zero for the new day; later boundaries refresh only their owning source. Deferred provider flushes publish that cache without rescanning, while `agent-presence update` performs an explicit full refresh. The standalone `usage` command always performs a live full scan, while the signature intentionally reflects the latest source-boundary snapshots. Supported models are priced from a bundled LiteLLM snapshot, including model-specific cache TTL rates and Codex's configured service tier (private models can still use pricing overrides); sources and the stale-badge `—` guard: [Token usage](https://agent-presence.vercel.app/guide/token-usage/).

...
Signature accounting keeps a per-window, per-source snapshot in state. Same-day session-boundary hooks refresh only the contribution owned by their source (`opencode` scans opencode, `codex` scans Codex, and so on), then the renderer aggregates the configured source snapshots. This prevents one agent's lifecycle event or scanner version from recalculating another agent's ledger. The first source boundary after midnight, or the first boundary when no aggregate timestamp exists, is the deliberate exception: it scans all built-ins once, giving inactive sources a current-day zero contribution so they cannot block the aggregate indefinitely. An explicit `agent-presence update` also scans and replaces the complete built-in snapshot set; deferred flushes and power-event resets only re-render the cache. A signature refresh is atomic across its configured windows and sources: if any scan fails, the previous cache remains intact for the next boundary to retry. Overlapping refreshes merge snapshots monotonically by scan time, so a slower older scan cannot overwrite newer usage. During migration from a legacy cache that already has a current aggregate timestamp but lacks some window snapshots, a partial source refresh preserves the previous badge and timestamp until every configured source has a compatible snapshot; it never presents a partial or cross-day value as the total.
...
Cost semantics live in `src/usage/pricing.ts`: Pi and opencode log a real per-message cost, which is trusted as-is (even `0`); Claude, Codex, Gemini, and TraeX-style handlers reprice by model id. Claude records one-hour cache creation separately from its default five-minute cache, so the scanner treats the TTL breakdown as authoritative when present, preserves the one-hour subset, and applies LiteLLM's dedicated one-hour rate. Codex pricing follows ccusage's `--speed auto` behavior: the scanner reads the current `service_tier` from `~/.codex/config.toml`, applies each model's fast-tier multiplier, and switches the entire request to the model's long-context rates when total prompt input crosses its threshold (272K for current GPT-5 long-context models). Codex usage events are also de-duplicated globally by timestamp, resolved model, and raw token buckets so copied branch/subagent history cannot be charged again from another session file. The primary price table is a committed **LiteLLM snapshot** (`src/usage/litellm-pricing.json`) generated by `pnpm run update-pricing`, filtered to the model ids agent-presence sources actually record (for example `gpt-5.6-sol`, `gpt-5.6-terra`, `claude-sonnet-5`, `deepseek-v4-pro`, `gemini-3-flash-preview`) rather than the full LiteLLM database. A weekly GitHub Action regenerates the snapshot and opens a PR when those supported-model prices drift. Resolution order is user `config.usage.pricing` overrides, then the LiteLLM snapshot, then the small fallback `DEFAULT_PRICING` table. Unknown models still show `n/a` while token counts stay exact. Full design: [`rfcs/source-usage.md`](../rfcs/source-usage.md).
